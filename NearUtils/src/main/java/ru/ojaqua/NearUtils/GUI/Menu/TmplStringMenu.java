...
import ru.ojaqua.NearUtils.Handlers.TmplStringHandler.TmplStringItemHandlerImpl;
...
public class TmplStringMenu extends Menu {
	...
	private void init() {
		...
		List<MenuItemParam> tmplStringPrmList = params.getStringHandlerPrm().getTmplStringList().stream()
				.map(str -> MenuItemParam.crExecuter(str, new TmplStringItemHandlerImpl(clipboard, str))).collect(Collectors.toList());
